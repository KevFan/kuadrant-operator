...
func (r *KuadrantStatusUpdater) updateKuadrantStatus(ctx context.Context, kObj *kuadrantv1beta1.Kuadrant) error {
	...
	_, err = r.Client.Resource(kuadrantv1beta1.KuadrantsResource).Namespace(kObj.GetNamespace()).ApplyStatus(ctx, kObj.GetName(), obj, metav1.ApplyOptions{FieldManager: FieldManager})
	return err
}
